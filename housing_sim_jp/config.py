...
def create_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--husband-nisa-used", type=float, default=None, help=f"夫の新NISA既投資元本・万円（旧NISAは含めない）(default: {d['husband_nisa_used']:.0f})")
    parser.add_argument("--wife-nisa-used", type=float, default=None, help=f"妻の新NISA既投資元本・万円（旧NISAは含めない）(default: {d['wife_nisa_used']:.0f})")
    parser.add_argument("--husband-nisa-balance", type=float, default=None, help="夫の新NISA現在評価額・万円（省略時=元本と同額）")
    parser.add_argument("--wife-nisa-balance", type=float, default=None, help="妻の新NISA現在評価額・万円（省略時=元本と同額）")
