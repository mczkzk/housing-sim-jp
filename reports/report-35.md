...
| いじめ・学区問題 | 学区が住所に固定。転校には売却を伴う引越しが必要（子1人×小1〜高3で約11年間さらされる） | 学区を変える引越しが現実的な選択肢 |
...
本シミュレーションでの80歳時点の不動産比率（税引前）: 浦和マンション 7%、浦和一戸建て 8%。
...
シミュレーション出力は80歳時点の名目値。2.0%インフレ×45年で割り引き、2026年実質値に変換（係数0.41）。「必要資産」は年金収入42.6万円/月を控除した本世帯の実質必要額。

| 戦略 | 確定論（実質） | グレード | MC P50（実質） | グレード | MC P25（実質） | グレード |
|------|-------------|---------|-------------|---------|-------------|---------|
| 浦和マンション | 1.61億 | **A** | 1.15億 | B | 0.24億 | D |
| 浦和一戸建て | 2.18億 | **A** | 1.52億 | A | 0.35億 | C |
| 戦略的賃貸 | 1.83億 | **A** | 0.92億 | B | 0.09億 | - |
| 通常賃貸 | 1.28億 | **B** | 0.51億 | C | 0.00億 | - |
...
**特別支出：** 飛鳥日本一周（名目計1,020万円）を織り込み済み。最良戦略の最終資産の約2%に相当し、最終資産に対して十分許容範囲。ただし破綻確率が高い状況では、特別支出の優先度を再検討する余地がある。
